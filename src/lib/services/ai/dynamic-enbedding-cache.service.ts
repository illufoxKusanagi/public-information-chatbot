...
export class DynamicEmbeddingCacheHelper {
  ...
  private async searchInternalData(query: string, limit: number) {
    try {
      ...
      // Fix: Use proper vector casting and cosine distance
      ...
      const results = await db
        .select({
          ...
          data: ragData.data,
          similarity: sql<number>`1 - (${ragData.embedding} <=> ${embeddingString}::vector)`,
        ...
        .where(
          and(
            sql`1 - (${
              ragData.embedding
            } <=> ${embeddingString}::vector) > ${parseFloat(
        ...
        .orderBy(
          sql`1 - (${ragData.embedding} <=> ${embeddingString}::vector) DESC`
        )
        .limit(limit);
